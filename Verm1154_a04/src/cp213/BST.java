...
public class BST<T extends Comparable<T>> {
	...
	boolean isDataEqual = source.getData() == target.getData();
	boolean isHeightEqual = source.getHeight() == target.getHeight();

	if (isDataEqual && isHeightEqual) {
	    boolean isLeftEqual = equalsAux(source.getLeft(), target.getLeft());
	    boolean isRightEqual = equalsAux(source.getRight(), target.getRight());
	...
	boolean isRightValid = isValidAux(node.getRight(), maxNode, node);
	...
	if (temp < 0) {
		...
		node = new TreeNode<>(new CountedData<>(next.getData()));
		node.setRight(removeAux(node.getRight(), next.getData()));
	    }
	}
