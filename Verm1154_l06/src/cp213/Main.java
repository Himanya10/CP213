...
public class Main {
	...
	while (proceed) {
	    ...
	    try {
		// Add the entered integer to the total
		total += keyboard.nextInt();
	    } catch (InputMismatchException e) {
		String input = keyboard.next(); // Capture the non-integer input
		if (input.equalsIgnoreCase("Quit")) {
		    proceed = false; // End loop if input is "Quit"
		} else {
     ...
     */
    public static String stringPrinter(int n, String str) throws IllegalArgumentException {

	String repeat = ""; // Initialize without a space
